...
public class cCharacteristicp : MonoBehaviour
{
    ...
    protected cAutoDetectionp myDetection
    {
        get // 사용할 때 바로 적용되도록 property 사용
        {
            if (_Detection == null) _Detection = this.GetComponent<cAutoDetectionp>(); // _anim을 호출하면 항상 값을 가지고 있다는 확신이 생김 

            if (_Detection == null) _Detection = this.GetComponentInChildren<cAutoDetectionp>(); // 자식에 있을때도 호출
